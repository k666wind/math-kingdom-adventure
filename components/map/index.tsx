...
const RegionDetail: React.FC<{region: Region; onClose: ()=>void}> = ({region, onClose}) => {
  ...
  const battleRecords  = useGameStore(s => s.player?.battleRecords ?? {})
  ...
  const StarRow: React.FC<{ stars: 0|1|2|3 }> = ({ stars }) => (
    <div className="flex gap-0.5">
      {[1,2,3].map(n => (
        <span key={n} style={{ fontSize: 12, opacity: n <= stars ? 1 : 0.2 }}>⭐</span>
      ))}
    </div>
  )
      ...
      <div className="w-full max-w-[480px] mx-auto rounded-t-3xl p-5 pb-8 max-h-[80vh] overflow-y-auto"
        ...
        <div className="flex flex-col gap-2 mb-5">
          {region.battles.map(b => {
            ...
            const record  = battleRecords[b.id]
            const stars   = (record?.bestStars ?? 0) as 0|1|2|3
