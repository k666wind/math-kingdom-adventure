...
export const BattleScreen: React.FC = () => {
  ...
  // ── Victory Screen ──────────────────────────────────────────
  if (isVictory) {
    const accuracy = battle.questionsAnswered.length > 0
      ? Math.round((battle.questionsAnswered.filter(r => r.isCorrect).length / battle.questionsAnswered.length) * 100)
      : 100
    ...
    return (
      <div className="h-full flex flex-col items-center px-5 py-8 animate-slide-up"
        ...
        <p className="font-nunito text-sm mb-5" style={{ color: 'rgba(255,255,255,0.6)' }}>
        ...
        <div className="w-full rounded-2xl p-4 mb-4"
          ...
          {[
            { label: 'Questions answered', val: String(battle.questionsAnswered.length) },
            { label: 'Accuracy', val: `${accuracy}%`, green: accuracy >= 80 },
            { label: 'Perfect battle?', val: battle.questionsAnswered.every(r => r.isCorrect) ? '✓ Yes! +30g' : 'No' },
          ].map(row => (
            <div key={row.label} className="flex justify-between py-1.5"
              ...
              <span className="font-nunito text-sm font-bold"
                style={{ color: row.green ? '#6BCB77' : 'white' }}>{row.val}</span>
            </div>
          ))}
        </div>
