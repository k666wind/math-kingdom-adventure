...
export const AchievementsScreen: React.FC = () => {
  ...
  if (player.completedBattles.length >= 1) unlocked.add('first_win')
  if (player.level >= 5)  unlocked.add('level5')
  if (player.level >= 10) unlocked.add('level10')
  if (player.level >= 20) unlocked.add('level20')
  if (player.ownedPets.length >= 1) unlocked.add('pet1')
  if (Object.values(player.equippedItems).some(Boolean)) unlocked.add('equip1')
      ...
      <div className="px-4 py-3" style={{ background: '#2D1B69', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
        <div className="h-2.5 rounded-full overflow-hidden" style={{ background: 'rgba(255,255,255,0.15)' }}>
          <div className="h-full rounded-full" style={{
            width: `${(unlocked.size/ACHIEVEMENTS.length)*100}%`, background: '#FFE66D' }}/>
        </div>
        ...
        {ACHIEVEMENTS.map(a => {
          ...
          return (
            <div key={a.id} className="rounded-2xl p-4 flex items-center gap-4"
              style={{ background: done ? 'white' : 'rgba(255,255,255,0.5)',
                       border: done ? '1px solid rgba(45,27,105,0.1)' : '1px dashed rgba(45,27,105,0.1)',
                       opacity: done ? 1 : 0.5 }}>
              <span className="text-3xl" style={{ filter: done ? 'none' : 'grayscale(1)' }}>{a.emoji}</span>
              <div className="flex-1">
                <div className="font-fredoka text-base" style={{ color: done ? '#2D1B69' : '#aaa' }}>{a.name}</div>
                <div className="font-nunito text-xs mt-0.5" style={{ color: '#aaa' }}>{a.desc}</div>
              </div>
